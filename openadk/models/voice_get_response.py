...
class VoiceGetResponse(object):
    ...
    def status(self):
        """Gets the status of this VoiceGetResponse.  # noqa: E501
         ...
         All scenarios: If the task is auto transform (iat), the status value as below: - idle - run  If the task is auto speech recognition (asr), the status value as below: - idle - run  If the task is text to speech (tts), the status value as below: - idle - run - build - wait   # noqa: E501
